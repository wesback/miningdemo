...
Based on: github.com/wesback/miningdemo

Scenario
You are a mining operations engineer at a fictional Pilbara (Western Australia) iron ore mine. The operation runs 24/7 across three zones — Zone-A, Zone-B, Zone-C — with a fleet of:
...
Every sensor publishes a JSON event every 10 seconds to an Event Hub / Eventstream. The goal is: ingest → store → query → visualise → alert — entirely in Microsoft Fabric.
...
Open app.fabric.microsoft.com and create a new workspace named MiningRTI-Demo with your Fabric capacity assigned.
Inside the workspace: + New → Eventhouse → name it MiningRTI.
A KQL database named MiningRTI is created automatically — rename it to MiningOps.
Note the Query URI shown on the database overview page (you'll need it for the Eventstream destination).
Quick deployment option: The repo includes deploy.py — a single script that creates all Fabric items via the REST API. Run it if you want to skip the manual steps:

pip install azure-identity requests
python3 deploy.py --workspace-id <your-workspace-guid>
After it completes, you still need to wire up the Eventstream source and destination in the portal UI.

Step 2 — Set Up the KQL Schema
Open kql/01-schema-setup.kql in your editor and run each command block one at a time in a KQL Queryset connected to MiningOps.

Important: KQL Querysets execute one control command per run. Select from the . to the end of the block, then click Run.
...
The update policy pattern — data lands in SensorReadings once; Fabric automatically routes copies to the right table with zero code:
...
+ New → Eventstream → name it MiningSensorStream. Enable Enhanced capabilities.

Add Source:

Choose Custom endpoint (provisions an Event Hub-compatible endpoint inside Fabric — no separate Azure subscription needed)
...
Click Activate to start the pipeline.

No transformations are needed in the Eventstream itself — the update policies in the Eventhouse handle all fan-out and routing automatically.

Step 4 — Start the Simulator
The simulator (simulator/simulator.py) generates realistic sensor events for all 15 devices in the fleet.
...
Both values come from the Custom endpoint source in your Eventstream. Open the source in the Fabric portal and copy the connection string and the Event Hub name — the name is a system-assigned GUID (e.g. es_xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx), not a human-readable label.
...
This generates historical_data/SensorReadings.csv and SafetyIncidents.csv (17 pre-correlated incidents). Ingest them via KQL:
...
Open a KQL Queryset connected to MiningOps. All named queries below are defined in kql/03-queries.kql.
...
+ New → Real-Time Dashboard → name it Mining Operations. Connect MiningOps as a data source. Set base auto-refresh to 30 seconds.
...
Set Latitude field → Latitude, Longitude field → Longitude, Bubble colour → by EquipmentType. Tooltip: EquipmentId, Make, Model, Zone.
...
+ New → Reflex → name it Mining Safety Alerts. Full rule definitions are in activator/alert-rules.md.
...
Cooldown: 5 minutes
Actions:
Teams message to #Mining Safety Channel: "🚨 CRITICAL: CO Level Breach — Zone: {{Zone}}, Reading: {{Value}} ppm"
...
Rules 6 & 7
Rule 6 — Truck Health Critical: fires when HealthScore < 40 (uses the composite score query from Step 5).
Rule 7 — Conveyor Stoppage: detects belt_speed_m_s == 0 for 60+ continuous seconds.
Step 8 — Anomaly Detection (No-Code ML)
Enable the Python plugin on your Eventhouse: Eventhouse → Plugins → enable Python 3.11.7 DL.

Allow up to 1 hour for the plugin to activate the first time.

Open Real-Time Hub → find EquipmentTelemetry → Anomaly detection.
...
Click Run analysis — the system tests multiple algorithms and ranks them (up to 4 minutes).

Review recommended models, adjust sensitivity (Low / Medium / High), and Publish to Real-Time Hub.

Connect the anomaly detector to Activator so an alert fires whenever a new anomaly is detected.

For power users, the underlying KQL functions are also directly available (kql/04-predictive-queries.kql):
...
Tutorial based on github.com/wesback/miningdemo — Microsoft Fabric Real-Time Intelligence, March 2026.
